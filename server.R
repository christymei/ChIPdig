    ...
    if(input$first_choice=="2") {
      ...
      if(input$first_choice=="3") {
        ...
        if(input$first_choice=="1") {
          ...
          output$file_input <- renderUI(fileInput('file_input', 'Select tab-delimited text file listing sequence files (see instructions):',multiple = FALSE, accept='text/comma-separated-values,text/plain'))
          updateTabsetPanel(session, "inTabset",selected = "map")
          output$text1 <- renderUI(NULL)
          output$text2 <- renderUI( h5("The alignment module requires a tab-delimited text file listing read files. Files ending with '.fq', '.fastq', '.gz', '.bz2' and 'xz' are supported. For a single-read experiment, the tab-delimited text file is merely a list of the names of files to be analyzed. Do not assign column names. Here's an example:"))
